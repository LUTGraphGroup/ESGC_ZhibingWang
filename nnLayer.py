...
class MLP_old(nn.Module):
    def __init__(self, inSize, outSize, hiddenSizeList=[], dropout=0.0, name='MLP', actFunc=nn.ReLU, bn=False,
                 outAct=False):
...
# 使用Tanh激活
class TextCNN(nn.Module):
    def __init__(self, featureSize, filterSize, contextSizeList, reduction='pool', actFunc=nn.ReLU, bn=False,
                 name='textCNN'):
        super(TextCNN, self).__init__()
        moduleList = []
        for i in range(len(contextSizeList)):
...
class TextLSTM(nn.Module):
    def __init__(self, feaSize, hiddenSize, num_layers=1, dropout=0.0, bidirectional=True, name='textBiLSTM'):
        ...
        self.name = name
        self.biLSTM = nn.LSTM(feaSize, hiddenSize, bidirectional=bidirectional, batch_first=True, num_layers=num_layers,
                              dropout=dropout)
    ...
    def forward(self, x, xlen=None):
        ...
        output, hn = self.biLSTM(
            x)  # output: batchSize × seqLen × hiddenSize*2; hn: numLayers*2 × batchSize × hiddenSize
        if xlen is not None:
...
class TextGRU(nn.Module):
    def __init__(self, feaSize, hiddenSize, num_layers=1, dropout=0.0, bidirectional=True, name='textBiGRU'):
        ...
        self.name = name
        self.biGRU = nn.GRU(feaSize, hiddenSize, bidirectional=bidirectional, batch_first=True, num_layers=num_layers,
                            dropout=dropout)
...
class ICDCandiAttention(nn.Module):
    ...
    def forward(self, X, candidate):
        # X: batchSize × seqLen × inSize
        batchLabelVec = torch.stack([self.U.weight[candidate[i]] for i in range(len(candidate))],
                                    dim=0)  # => batchSize × Candidates num(1000) × inSize
        alpha = F.softmax(torch.matmul(X, batchLabelVec.transpose(1, 2)),
                          dim=1)  # => batchSize × seqLen × Candidates num(1000)
...
class DeepICDDescAttention(nn.Module):
    def __init__(self, inSize, classNum, labSize=896, hdnDropout=0.1, attnList=[], labDescVec=None, name='DeepICDAttn'):
        super(DeepICDDescAttention, self).__init__()
        hdns,attns,bns = [],[],[]
        for i,os in enumerate(attnList):
            print(f"  Linear {i}: in_features={inSize}, out_features={os}")
            attns.append(nn.Linear(inSize,os))
            if i==len(attnList)-1:
...
class DeepICDDescCandiAttention(nn.Module):
    def __init__(self, inSize, classNum, labSize=768, hdnDropout=0.1, attnList=[], labDescVec=None, name='DeepICDDescCandiAttention'):
        super(DeepICDDescCandiAttention, self).__init__()
        hdns, attns, bns = [], [], []
        for i, os in enumerate(attnList):
            attns.append(nn.Linear(inSize, os))
            if i == len(attnList) - 1:
                hdns.append(nn.Linear(inSize, inSize))  # 保持维度不变，后续统一映射
            else:
                hdns.append(nn.Linear(inSize, inSize))
            bns.append(nn.BatchNorm1d(inSize))
        self.hdns = nn.ModuleList(hdns)
        ...
        self.dropout = nn.Dropout(p=hdnDropout)
        self.labDescVec = nn.Parameter(torch.tensor(labDescVec, dtype=torch.float32)) if labDescVec is not None else None
        self.final_proj = nn.Linear(inSize, labSize)  # 映射到与标签描述一致的维度
        self.name = name

    def forward(self, X, candidate, labDescVec=None):
        if labDescVec is None:
            labDescVec = self.labDescVec  # [classNum, labSize]

        if isinstance(candidate, np.ndarray):
            candidate = torch.from_numpy(candidate).long()
        elif isinstance(candidate, list):
            candidate = [c.long() if torch.is_tensor(c) else torch.tensor(c, dtype=torch.long) for c in candidate]
        ...
        X = self.final_proj(X)  # [B, os, labSize]

        if isinstance(candidate, list):
            batchLabelVec = torch.stack([labDescVec[c] for c in candidate], dim=0)
        elif isinstance(candidate, torch.Tensor):
            if candidate.dim() == 2:
                batchLabelVec = torch.stack([labDescVec[c] for c in candidate], dim=0)
            elif candidate.dim() == 1:
                B = X.size(0)
                labelVec = labDescVec[candidate]
                batchLabelVec = labelVec.unsqueeze(0).expand(B, -1, -1)
            else:
                raise ValueError(f"Unsupported candidate shape: {candidate.shape}")
        else:
            raise TypeError("Unsupported type for candidate")

        alpha = F.softmax(torch.matmul(X, batchLabelVec.transpose(1, 2)), dim=1)
        X = torch.matmul(alpha.transpose(1, 2), X)
        return X
...
class InterationAttention(nn.Module):
    ...
    def concat_simple_attention(self, x, y):
        ...
        u = torch.cat([x, y_pooled.unsqueeze(dim=1).expand(-1, x.shape[1], -1)],
                      dim=2)  # => batchSize × seqLen1 × (feaSize1+feaSize2)
        v = torch.cat([y, x_pooled.unsqueeze(dim=1).expand(-1, y.shape[1], -1)],
                      dim=2)  # => batchSize × seqLen2 × (feaSize1+feaSize2)
    ...
    def plane_attention_s(self, x, y):
        ...
        x, y = x.unsqueeze(dim=2).expand(-1, -1, y.shape[1], -1), y.unsqueeze(dim=1).expand(-1, x.shape[1], -1,
                                                                                            -1)  # => batchSize × seqLen1 × seqLen2 × feaSize
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, xlen=None):
        ...
        scores = [torch.bmm(queries[i], keys[i].transpose(1, 2)) / np.sqrt(self.dk) for i in
                  range(self.multiNum)]  # => multiNum*(batchSize × seqLen × seqLen)
        # mask <EOS> padding
        if xlen is not None:
            for i in range(len(scores)):
                mask = torch.zeros(scores[0].shape, dtype=torch.float32,
                                   device=scores[i].device)  # => batchSize × seqLen × seqLen
                for j, k in enumerate(xlen):
                    mask[j, :, k - 1:] -= 999999
                scores[i] = scores[i] + mask
        z = [torch.bmm(F.softmax(scores[i], dim=2), values[i]) for i in
             range(self.multiNum)]  # => multiNum*(batchSize × seqLen × dk)
...
class TextTransformer(nn.Module):
    def __init__(self, seqMaxLen, layersNum, featureSize, dk, multiNum, dropout=0.1, name='textTransformer'):
        ...
        posEmb = [[np.sin(pos / 10000 ** (2 * i / featureSize)) if i % 2 == 0 else np.cos(
            pos / 10000 ** (2 * i / featureSize)) for i in range(featureSize)] for pos in range(seqMaxLen)]
...
class Transformer_Wcnn(nn.Module):
    ...
    def forward(self, x):
        ...
        score = [torch.bmm(queries[i], keys[i].transpose(1, 2)) / np.sqrt(self.dk) for i in
                 range(self.multiNum)]  # => multiNum*(batchSize × seqLen × seqLen)
        z = [torch.bmm(F.softmax(score[i], dim=2), values[i]) for i in
             range(self.multiNum)]  # => multiNum*(batchSize × seqLen × dk)
        z = self.WO(torch.cat(z, dim=2))  # => batchSize × seqLen × feaSize
        z = x + self.dropout(self.layerNorm1(z))  # => batchSize × seqLen × feaSize
...
class TextTransformer_Wcnn(nn.Module):
    def __init__(self, layersNum, featureSize, dk, multiNum, seqMaxLen, dropout=0.1, name='textTransformer'):
        ...
        self.transformerLayers = nn.Sequential(
            OrderedDict(
                [('transformer%d' % i, Transformer_Wcnn(featureSize, dk, multiNum, seqMaxLen, dropout)) for i in
                 range(layersNum)]
            )
        )
        self.dropout = nn.Dropout(p=dropout)
...
class HierarchicalSoftmax(nn.Module):
    def __init__(self, inSize, hierarchicalStructure, lab2id, hiddenList1=[], hiddenList2=[], dropout=0.1,
...
class FocalCrossEntropyLoss(nn.Module):
    ...
    def forward(self, Y_pre, Y):
        ...
        if not torch.is_tensor(Y):
            Y = torch.tensor(Y, dtype=torch.long, device=Y_pre.device)
        else:
...
class MultiLabelFocalLoss(nn.Module):
    def __init__(self, gamma=2, weight=None, reduction='mean', logit=True):
...
class MultiTaskCEL(nn.Module):
    def __init__(self, lossBalanced=True, ageW=1, genderW=1, name='MTCEL'):
        ...
        self.genderS, self.ageS = nn.Parameter(torch.zeros(1, dtype=torch.float),
                                               requires_grad=lossBalanced), nn.Parameter(
    ...
    def forward(self, genderY_logit, genderY, ageY_logit, ageY):
        if self.lossBalanced:
            return self.genderW * torch.exp(-self.genderS) * self.genderCriterion(genderY_logit,
                                                                                  genderY) + self.ageW * torch.exp(
                -self.ageS) * self.ageCriterion(ageY_logit, ageY) + (self.genderS + self.ageS) / 2
        else:
            return self.genderW * self.genderCriterion(genderY_logit, genderY) + self.ageW * self.ageCriterion(
                ageY_logit, ageY)
...
class flash_linear_trans(nn.Module):
    ...
    def forward(self, input, prev=None):
        ...
        real_seq_len = x.shape[1]  # 保留原始长度以便后续 residual connection
        padding = padding_to_multiple_of(self.seqMaxLen, self.chunk_length)

        if padding > 0:
            x = F.pad(x, (0, 0, 0, padding))  # 在 seqLen 维度 pad
        ...
        x = torch.cat([chunk.unsqueeze(1) for chunk in chunks], dim=1)  # shape: [b, g, c, d]
        _, g, c, d = x.shape

        # Feature projection
        U, V, base = torch.split(self.actFun(self.UV(x)), [self.e, self.e, self.s], dim=-1)
        base = einsum('...r, hr->...hr', base, self.gamma) + self.beta
        base = rope_flash(base, axis=[1, 2])
        ...
        A = F.softmax(prev, dim=-1)
        A = self.dropout(A)
        quadratic = torch.matmul(A, V)

        lin_kv = torch.matmul(lin_k.transpose(2, 3), V)
        linear = torch.matmul(lin_q, lin_kv) / self.seqMaxLen

        # flatten back to [batch, seqLen, dim] and truncate to real_seq_len
        U, quadratic, linear = map(
            lambda t: rearrange(t, 'b g n d -> b (g n) d')[:, :real_seq_len],
            (U, quadratic, linear)
        )
        ...
        x = self.layerNorm(input + self.dropout(x))  # [batch, seqLen, embSize]
...
class MoEExpert(nn.Module):
    """单个MoE专家 - 专门处理特定医学领域"""
    def __init__(self, d_model, d_ff, dropout=0.1, specialty=None):
        super().__init__()
        ...
        # 根据专业领域初始化权重
        if specialty:
            self._init_specialty_weights()
    
    def _init_specialty_weights(self):
        """根据专业领域初始化权重"""
        if self.specialty == "cardiology":
            # 心脏病相关模式
            self.specialty_patterns = ["胸痛", "心电图", "心肌梗死", "心律失常", "冠心病"]
        elif self.specialty == "diabetes":
            # 糖尿病相关模式
            self.specialty_patterns = ["血糖", "胰岛素", "糖尿病", "糖化血红蛋白", "酮症"]
        elif self.specialty == "infection":
            # 感染病相关模式
            self.specialty_patterns = ["发热", "白细胞", "感染", "抗生素", "炎症"]
        elif self.specialty == "oncology":
            # 肿瘤相关模式
            self.specialty_patterns = ["肿瘤", "癌症", "化疗", "放疗", "转移"]
        else:
...
class MoELayer(nn.Module):
    """MoE层 - 动态路由到不同专家"""
    def __init__(self, d_model, num_experts=8, num_experts_per_token=2, 
                 expert_capacity_factor=1.0, d_ff=2048, dropout=0.1,
                 use_shared_experts=True, num_shared_experts=2,
                 specialties=None):
        super().__init__()
        ...
        if specialties:
            self.experts = nn.ModuleList([
                MoEExpert(d_model, d_ff, dropout, specialty=specialties[i])
                for i in range(num_experts)
            ])
        else:
            self.experts = nn.ModuleList([
                MoEExpert(d_model, d_ff, dropout)
        ...
        if use_shared_experts:
            self.shared_experts = nn.ModuleList([
                MoEExpert(d_model, d_ff, dropout, specialty="general")
        ...
        self.register_buffer('expert_usage_buffer', torch.zeros(num_experts))
    ...
    def forward(self, x):
        ...
        # 计算专家容量
        expert_capacity = int(
            (batch_size * seq_len * self.num_experts_per_token) / self.num_experts * self.expert_capacity_factor
        )
        ...
        for expert_idx in range(self.num_experts):
            ...
            # 回填到原始位置
            final_output[expert_mask] = weighted_output[:expert_mask.sum()]
            
            # 更新专家使用统计
            self.expert_usage_buffer[expert_idx] += expert_mask.sum().item()
    ...
    def get_aux_loss(self):
        ...
        if self.training:
            # 计算专家使用频率
            total_usage = self.expert_usage_buffer.sum()
            if total_usage > 0:
                expert_freq = self.expert_usage_buffer / total_usage
                target_freq = 1.0 / self.num_experts
                
                # 负载均衡损失
                aux_loss = F.mse_loss(expert_freq, torch.full_like(expert_freq, target_freq))
                return aux_loss
        return 0.0

class MedicalMoERouter(nn.Module):
    """医疗文本专用路由器 - 根据医学词汇选择专家"""
    def __init__(self, d_model, num_experts, specialties):
        super().__init__()
        self.num_experts = num_experts
        ...
        self.medical_patterns = {
            "cardiology": ["胸痛", "心电图", "心肌梗死", "心律失常", "冠心病", "心绞痛", "心力衰竭"],
            "diabetes": ["血糖", "胰岛素", "糖尿病", "糖化血红蛋白", "酮症", "多饮", "多尿"],
            "infection": ["发热", "白细胞", "感染", "抗生素", "炎症", "细菌", "病毒"],
            "oncology": ["肿瘤", "癌症", "化疗", "放疗", "转移", "恶性", "良性"],
            "neurology": ["头痛", "癫痫", "中风", "脑梗塞", "神经系统"],
            "respiratory": ["咳嗽", "呼吸困难", "肺炎", "支气管炎", "哮喘"],
            "gastroenterology": ["腹痛", "恶心", "呕吐", "腹泻", "胃炎", "溃疡"],
            "general": ["患者", "检查", "治疗", "症状", "诊断"]
        }
    ...
    def forward(self, x, text_tokens=None):
        ...
        if text_tokens is not None:
            ...
            for i, specialty in enumerate(self.specialties):
                if specialty in self.medical_patterns:
                    patterns = self.medical_patterns[specialty]
                    # 检查文本中是否包含该专业的词汇
